...
public class UserDAO2 {
	...
	public UserDAO2() {
		try {
			...
			Context envContext = (Context) initContext.lookup("java:/comp/env"); //실질적으로 소스에 접근
	...
	public int login(String userID, String userPassword) {
		...
		PreparedStatement pstmt = null; //sql 인젝션 같은 해킹을 방어해주고 안정적인 sql 사용가능하게함
		ResultSet rs = null;
		...
		try {
			...
			if(rs.next()) {
				...
				if(rs.getString("userProfile").equals("")) {
					...
					return "http://localhost:8080/UserChat/images/icon.jpg";
				}
			System.out.println(rs.getString("userProfile"));
				return "http://localhost:8080/UserChat/images/" + rs.getString("userProfile");
			}
